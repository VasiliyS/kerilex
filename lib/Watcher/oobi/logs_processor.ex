defmodule Watcher.OOBI.LogsProcessor do
  ...
  @typedoc """
  map that will be used to log a structured message
  """
  ...
  defp process_escrow(escrow, pre_said_key, state_cache) do
    if EventEscrow.has_events_waiting_for?(escrow, pre_said_key) do
      {:ok, events, escrow} = EventEscrow.pop_events_waiting_for(escrow, pre_said_key)

      events
      |> Enum.reduce_while(
        {:ok, escrow, state_cache},
        &escrow_event_reducer(&1, &2, pre_said_key)
      )
    else
      {:ok, escrow, state_cache}
    end
  ...
  defp escrow_event_reducer({msg_obj, parsed_event}, {:ok, escrow, state_cache}, said_key) do
    ...
    with {:ok, said, new_state, response} <-
         ...
         {:ok, escrow, state} <-
           (
             Logger.debug(Map.put(response, :msg, "added event from escrow"))

             process_escrow(escrow, said, new_state)
           ) do
  ...
  defp do_process_msg(type, msg_obj, parsed_msg_map, state_cache)
    ...
    with {:ok, sn} <-
         ...
         :ok <- check_config_and_parent(msg_obj) do
      # at this point we know that this event is allowed and it's parent is already processed
      # we are now ready to see if this `sn` is already present and might be a duplicate or a superseding recovery!
  ...
  defp maybe_update_kel(type, msg_obj, parsed_msg_map, state_cache) do
    ...
    case check_for_duplicity(msg_obj) do
      :ok ->
        maybe_store_msg(type, nil, msg_obj, parsed_msg_map, state_cache)
      ...
      duplicity_res ->
        # this can only be duplicity, so passing on to log processing loop
        duplicity_res
    end
  end

  defp check_for_duplicity(msg_obj) do
    pre = msg_obj["i"]
    sn = msg_obj["s"]
    ...
    case KeyStateStore.find_event(pre, sn) do
      ...
      {:key_event_found, found_event} ->
        cond do
          msg_obj["d"] == found_event["d"] ->
            :already_exists

          msg_obj["t"] in ~w|icp dip ixn| ->
  ...
  defp check_config_and_parent(msg_obj) do
    pref = msg_obj["i"]
    sn = msg_obj["s"]

    with :ok <- if(sn > 0, do: KeyStateStore.validate_config(pref, msg_obj), else: :ok),
         :ok <- if(sn > 0, do: KeyStateStore.check_parent(pref, sn, msg_obj), else: :ok) do
      :ok
    else
      :no_icp_event ->
  ...
  @spec maybe_store_msg(
          ...
          {:ok, Kerilex.said(), KeyStateCache.t(), log_response()}
  ...
  defp maybe_store_msg("ixn", nil, msg_obj, parsed_msg, state_cache) do
  ...
  defp maybe_store_msg("icp", nil, msg_obj, parsed_msg, state_cache) do
  ...
  defp maybe_store_msg("dip", nil, msg_obj, parsed_msg, state_cache) do
    with {:ok, sig_th} <- KELParser.check_sigs_on_stateful_msg(msg_obj, parsed_msg),
  ...
  defp maybe_store_msg("drt", nil, msg_obj, parsed_msg, state_cache) do
    with {:ok, drt_event} <-
  ...
  defp maybe_store_msg("rot", nil, msg_obj, parsed_msg, state_cache) do
  ...
  defp maybe_store_msg("rot", "ixn", msg_obj, parsed_msg, state_cache) do
    ...
    if KeyStateStore.has_event_after?(pre, sn, "rot") do
      ...
      # TODO(VS): add transaction wrapper here, otherwise failed clean_kel can result in inconsistent state
      # with {:ok, _said, _state, _res} = resp <-
      #        maybe_store_msg("rot", nil, msg_obj, parsed_msg, state_cache),
      #      :ok <- KeyStateStore.clean_kel(msg_obj["i"], msg_obj["s"] + 1, resp) do
      #   resp
      # end
  ...
  defp maybe_store_msg(type, nil, msg_obj, _parsed_msg, state_cache) do
  ...
  defp handle_update_kel_res(res, event_obj, state_cache) do
    case res do
      {:ok, said} ->
        ...
        {:ok, said, state_cache,
         %{type: event_obj["t"], result: "updated KEL", pre: pre, sn: event_obj["s"]}}
